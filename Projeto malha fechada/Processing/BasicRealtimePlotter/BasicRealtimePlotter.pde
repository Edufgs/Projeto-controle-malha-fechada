...
boolean mockupSerial = false; //false para comunicação serial
...
void setup() {
  ...
  if (!mockupSerial) {
    String serialPortName = Serial.list()[1];  //COM2
...
void draw() {
  ...
  if (mockupSerial || serialPort.available() > 0) {
    ...
    println(myString);  //Imprime na tela
...
void setChartSettings() {
  ...
  LineGraph.xLabel=" Tempo ";
  LineGraph.yLabel="Saida";
  LineGraph.Title="";  
  LineGraph.xDiv=20;  
  LineGraph.xMax=100; 
  LineGraph.xMin=0; 
